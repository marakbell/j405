...
ANSWER:
- After looking at the correlation matrix, it looks like independent variables like race (pct_white) and employment are most likely to be correlated with voter turnout. This is shown by the darkly shaded circles in the turnout rows, which also are the largest circles in the matrix, signifying the strength of the correlation.
- Some variables that seem to have little to no correlation with voter turnout are education (pct_bachelors_higher) and population.
- Looking at the scatterplots, pct_unemployment stands out the most to me to seem to have a linear relationship with voter turnout. From just looking at the other plots, it's hard to tell the strength of the relationships, but income looks random and lacks strength.
...
ANSWER:
- Looking at the RSE, the model deviates from the actual turnout by 2.2  percentage points of turnout. 
- A multiple R-squared of 0.8539 signifies that about 85% of the variation in turnout is described by this model, and an adjusted R-squared of 0.8023 signifies that 80% of the variation is explained by the model after accounting for the number of variables. Though lower, the overall percentage of variation explained is high, suggesting that the model is a good fit for the problem we are trying to solve.
- With an F-statistic at 16.55 on 6 and 17 DF and a p-value at 2.999e-06, an extremely low value, those values suggest that the model as a whole is useful for predicting voter turnout.
- Looking at the asterisks next to each independent variable, income, median age, and pct_white stand out to be statistically significant, looking at their p-values, all less than 0.05. For income, the value 9.663e-07 estimates that for each extra dollar of income, turnout increases by about 0.000000966;Median age estimate at 0.00341 suggests that there's higher voter turnout as the population ages; the pct-_white estimate also suggests that the greater percentage of white voters, the higher the turnout.
- This multiple variable model predicts turnout well at a high multiple R-squared and adjusted-R-squared value, signifying that at 80% of the variation in voter turnout is explained here after accounting for number of variables. 
...
ANSWER:
- The models with the lowest p-values (income, median_age, pct_white) were significantly better than those with p-values that weren't statistically significant (population, pct_bachelors_higher, pct_umemployed).
- The model that performed the best compared to the full model was the linear model looking at turnout and race (pct_white), with an adjusted R-squared value at 0.4569641, or 45% of the variance explained by the model. Though low, it is higher than all other single independent variables compared to the dependent variable. 
...
ANSWER:
- If I were working on a story looking at voter participation in my home county, Frederick, in Maryland, I would select three or four independent factors that I hypothesize are defining in voter participation in the area. 
- I'd collect data on these variables: gender, residential stability (measured by length of residence), and income. I'd look at the correlation between these independent variables and my dependent variable (voter participation).
- After looking at the coefficients, I'd be able to determine whether there is strong evidence that shows if any of these factors (gender, residential stability, and income) impact the number of percentage points signifying voter participation in the area.
