...
montco_inspections |> 
  ...
  labs(title = "Health Inspection Compliance Scores in Montgomery County", x = "Compliance Score", y = "Number of Inspections")
...
The standard deviation and histogram tell us that the 1) the distribution of the compliance scores is relatively clustered around the mean. This is because rather than spreading across the histogram, most data are closer together than a dispersed data set. The histogram also shows a left skew, meaning that most compliance scores are high, with a few lower outliers. The story here could be a solutions story about what health inspection regulations are doing right to perhaps explain the left skew in data, meaning that more establishments are in compliance than they are not.
...
```{r}
md_hs_participation <- md_hs_participation |>  
  mutate(boys_pct =  ((boys/total)*  100))

md_hs_participation |>
  summarize(correlation = cor(boys_pct, girls_pct, method="pearson"))
```
...
md_hs_participation |> 
  ggplot() +
  geom_point(aes(x=boys_pct, y=girls_pct)) +
  geom_smooth(aes(x=boys_pct, y=girls_pct), method="lm") +
  labs(title = "Comparing Participation Among Sex", x = "Boys Participation", y = "Girls Participation")
...
The -1 correlation coefficient (meaning that there's a perfect negative correlation between the two variables), reveals that gender equity in Maryland high school sports participation is far from perfect. This is shown in the scatterplot because as girls participation increase, boys participation decreases and vice versa. I interpret that school districts below the line have less participation overall vs. ones above the line. School districts that don't have all girls or all boys schools are worth examining further, especially when co-ed sports exist because that would mean that there is a deeper indicator for why there is such a polarizing correlation.
...
car_thefts <- car_thefts |>
  mutate(total_rate = (2022 + 2023)/population * 1000)
