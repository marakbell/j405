...
Data may not be normally distributed when looking at ages in a population. For example, in Japan, age groups would be skewed to the left, with a high population in older ages. This is because Japan's population is high in elderly populations because of declining birth rate and other factors. Another example might be household income in a specific area. For example, in New Orleans, the highest earners make a lot more than the median earner, making the distribution likely skewed to the right. 

A reporter working on a story about data that isn't normally distributed should try to find a factor that better explains the skewed distribution. For example, when looking at Japan's population by age group, one might look at the factors that come into play that explain the skew in the distribution, such as health choices, fertility rates, healthcare, and other factors.
...
**Answer**:
Maryland places in the 95th percentile for the 2024 season. This means that Maryland's score of service aces in the season is 95% higher than all given data in the season. The top 5% scores from other teams are higher than Maryland's score of service aces.
...
**Answer**:
The distribution is skewed left. This is because the tail of the data is on the left end, meaning that the bulk of the data is clustered on the right side. 68% of the data falls within 1 standard deviation of the mean. 
