...
ANSWER HERE
- Yes, this model is useful for what we are evaluating. This is indicated in our calculations, first looking at the Multiple R-squared value at 0.6232. This number indicates that roughly 62.32% of the variability in wins is explained through the model, a relatively high figure, suggesting that the model is a good fit for explaining the data.
- The adjusted R-squared value is still significant at 58.28% of variance being explained by the model after accounting for the number of variables used.
- The F-statistic being relatively high at 15.44 suggests that the groups we are looking at have large variation between each other, meaning that the differences are less likely due to random chance. There is also a low p-value at 4.058e-06. Looking at these two values, this model is statistically significant and useful. 
...
ANSWER HERE
Of these three factors, Points Scored has the most impact on wins. This is clear in the visualization, showing a dark blue shaded large circle. This signifies that near strong positive correlation between points scored and wins at 0.7078293. Yards Gained seems to not be impactful at all to the wins because of its low correlation at 0.4430188.

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

- I'd use this model to cover a highly successful football game by looking at three numeric factors and comparing them to the number of points scored in the game. 
- For example, I'd take number of touchdowns, number of interceptions, and number of penalties, and look at the correlation between those three factors (independent variable) and the one single dependent variable (the number of points scored in the game).
- After looking at the coefficients, I'd be able to determine whether there is strong evidence that shows if any of these factors impact the number of points won and by how much. 
